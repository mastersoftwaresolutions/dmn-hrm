angular.module('NerdCtrl', []).controller('NerdController', function($scope,$routeParams,$http) {
	...
	$scope.employee_id=id;
	  $http.get('/api/employee/'+id).success(function(data){
	  	...
	  	//console.log("Running" , typeof data , data.length , JSON.parse({}));
	  	data={};
	  	data.employee_name='{}';
	  	data.hr='{}';
	  	data.hr_document='{}';
...
//if(typeof data =="undefined")
	...
	$scope.save= function() {
	  ...
	  if(docstatus==1)
	  {
	  $scope.pending_section=$scope.pending_section+'       Documentation';$scope.status=0;
	  }
	  else
	  {
	 ...
	 var netstatus=0;
	  ...
	  if(netstatus==1)
	  {
	  $scope.pending_section=$scope.pending_section+'         Network';$scope.status=0;
	  }
	  else
	  {
	  	$scope.pending_section=$scope.pending_section;
...
if($scope.status==0)$scope.status='Pending';
	...
	$scope.pending_section="Completed";
}
